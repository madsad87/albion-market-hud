...
export async function GET(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const shouldAutoScan = parsed.scanMode === 'auto' && items.length === 0;
    const effectiveItems = shouldAutoScan ? fallbackAutoItems : items;
    ...
    const knownItemsForRequest = effectiveItems.filter((itemId) => knownItemIdSet.has(itemId)).length;
    const effectiveScanMode: OpportunitiesMeta['scanMode'] = shouldAutoScan ? 'auto' : 'manual';
    const scanSource: OpportunitiesMeta['scanSource'] = shouldAutoScan ? 'known-items-auto' : 'query-items';
    ...
    const meta: OpportunitiesMeta = {
      ...
      itemCatalog: {
        ...
        coveragePct: Number(((knownItemsForRequest / effectiveItems.length) * 100).toFixed(2))
      }
    };
